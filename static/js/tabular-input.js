	...
	class tabularInput {
		...
		addColumn(element) {
			element.find('tbody tr').each((x, y) => {
				$(y).find('td:last').after('<td><input type="text" class="csv_fields" /></td>')
			})
			this.config.columns++
		...
		addRow(element) {
			...
			for (let a = 0; a < this.config.columns; a++) {
				newRowInputs.push('<input type="text" class="csv_fields" />')
			}
		...
		setColumnHeads(element, heads) {
			...
			//console.log(headsHTML)
			element.find('thead').html(`<tr>${headsHTML}</tr>`)
		}

		addColumnHeads(element, heads) {
			if (typeof heads !== 'object') {
				return
			}
		...
		removeColumnHeads(element, heads) {
			if (typeof heads !== 'object') {
				return
			}
			...
			return this.each(() => {
				...
				while (currentRow < config.rows) {
					...
					for (let a = 0; a < config.columns; a++) {
						currentRowInputs.push('<input type="text" class="csv_fields" />')
					}
